...
const getters = {
  getAutomlList: state => {
    state.automlList.sort(function(a, b) {
      return parseFloat(b[state.sortKey]) - parseFloat(a[state.sortKey]);
    });
...
const actions = {
  ...
  async fetchAutomlData(context, params) {
    await http.useGet(port.category.automlList, params).then(res => {
      context.commit("setAutoml", res.data.data);
    });
  },
  // 具体模型结构数据
  async fetchAutomlModelGraph(context, params) {
    await http.useGet(port.category.automlGraph, params).then(res => {
      context.commit("setAutomlGraph", res.data.data);
    });
  },
  // automl执行
  async automlProcess(context, params) {
    await http.useGet(port.category.automlProcess, params).then(res => {});
  }
};
